...
import java.lang.Math;import java.lang.Override;import java.lang.Runnable;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if (changed || forceToRelayout) {
			...
			if (mCurrentImage >= 0 && mCurrentImage < mCount) {
				...
				int[] items = new int[BaseApplication.size];
//				System.out.println("---长度：" + BaseApplication.homes.size());
				for (int i =0;i< BaseApplication.size;i++){
					items[i] = getIndexForItem(i);
				}
...
//					});
	...
	public OnMovechangeListener getChangeLisener(){
